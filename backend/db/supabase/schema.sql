...
create table if not exists store_catalog_periods (
  id uuid primary key default gen_random_uuid(),
  item_id uuid not null references store_catalog_items(id) on delete cascade,
  code text not null,
  label text not null,
...
create table if not exists store_catalog_highlights (
...
create index if not exists idx_store_catalog_items_sort on store_catalog_items(sort_order, is_active);
create index if not exists idx_store_catalog_periods_item on store_catalog_periods(item_id, sort_order);
create index if not exists idx_store_catalog_highlights_item on store_catalog_highlights(item_id, sort_order);
create index if not exists idx_store_promo_codes_active on store_promo_codes(is_active, code);
create index if not exists idx_store_orders_created_at on store_orders(created_at desc);
create index if not exists idx_store_orders_nickname on store_orders(nickname);
...
drop trigger if exists trg_store_promo_codes_updated_at on store_promo_codes;
create trigger trg_store_promo_codes_updated_at
before update on store_promo_codes
for each row
execute function set_row_updated_at();
